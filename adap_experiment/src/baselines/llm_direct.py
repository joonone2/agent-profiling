...
from prompts.templates import (
    ...
    BASELINE_WITH_HISTORY_RATING_PROMPT,
    BASELINE_WITH_HISTORY_RANKING_PROMPT,
...
def baseline_predict_rating_with_history(
    user_feature_vector: pd.Series,
    user_history: list[dict],
    item_info: dict,
) -> float:
    """
    BASELINE_WITH_HISTORY_RATING_PROMPT 사용 — 19dim 피처 + 최근 시청 이력을 함께 제공.
    ...
    prompt = BASELINE_WITH_HISTORY_RATING_PROMPT.format(
        feature_text=feature_text,
    ...
    try:
        ...
        logger.warning("baseline_predict_rating_with_history: parse failed. Response: %s", response[:200])
        raise


def baseline_rank_with_history(
    user_feature_vector: pd.Series,
    user_history: list[dict],
    candidates: list[dict],
) -> list[int]:
    """
    BASELINE_WITH_HISTORY_RANKING_PROMPT 사용 (listwise, 후보 20개 배치로 한 번에 질의).
    """
    feature_text = _format_user_features(user_feature_vector)
    history_text = _format_history(user_history)
    candidates_text = "\n".join(
    ...
    prompt = BASELINE_WITH_HISTORY_RANKING_PROMPT.format(
        feature_text=feature_text,
        history_text=history_text,
        n=len(candidates),
        candidates_text=candidates_text,
